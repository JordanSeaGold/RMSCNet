...
if __name__ == '__main__':
    # 加载模型
    model = YOLO('RMSCNet/run/model_cfg/RMSCNet.yaml')  # 配置文件路径，建议使用绝对路径
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data='Dataset/yolo_format/PIDray/PIDray.yaml',#数据集配置文件路径，建议使用绝对路径
        epochs=300,  # (int) 训练的周期数
        patience=300,  # (int) 等待无明显改善以进行早期停止的周期数
        ...
        project='',  # 训练结果保存路径（可选）
        name=r'',  # (str, optional) 实验名称，结果保存在'project/name'目录下
        ...
        deterministic=False,  # (bool) 是否启用确定性模式
        ...
        profile=True,  # (bool) 在训练期间为记录器启用ONNX和TensorRT速度
